...
function ConvertTo-Hashtable($obj) {
  ...
  if ($obj.PSObject -and $obj.PSObject.Properties.Count -gt 0) {
    $h = @{}
    foreach ($prop in $obj.PSObject.Properties) { $h[$prop.Name] = ConvertTo-Hashtable $prop.Value }
    return $h
  }
  return $obj
}
...
function Run-Action($Action, $Labels, [int]$Pc) {
  ...
  switch ($Action.type) {
    ...
    'data_list' {
      ...
      $targetName = if ($p.target) { [string]$p.target } else { [string]$p.name }
      $script:Vars[$targetName] = $cur
...
$btnApply.Add_Click({
  ...
  try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($txtParams.Text)) {
      $paramsObj = ConvertTo-Hashtable ($txtParams.Text | ConvertFrom-Json)
    }
...
$btnOpen.Add_Click({
  ...
  if ($dlg.ShowDialog() -eq 'OK') {
    try {
      ...
      $script:Macro = ConvertTo-Hashtable ($raw | ConvertFrom-Json)
