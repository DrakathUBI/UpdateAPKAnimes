...
func (r *Runner) execAction(index int, a Action) (int, bool, error) {
	...
	case "wait_text_input":
		needle := strings.ToLower(r.resolveValue(p["text"]))
		fmt.Printf("Digite texto para continuar (esperando: %s): ", needle)
		...
		for {
			...
			fmt.Print("Valor diferente, tente novamente: ")
		}
	case "wait_pixel_change", "wait_for_pixel_color_change", "wait_desktop_change", "find_image", "find_text", "find_text_ocr":
		timeout := parseInt(p, "timeout_ms", 5000)
		time.Sleep(time.Duration(timeout) * time.Millisecond)
		if xVar := strings.TrimSpace(p["x_var"]); xVar != "" {
			r.vars[xVar] = p["x"]
		}
		if yVar := strings.TrimSpace(p["y_var"]); yVar != "" {
			r.vars[yVar] = p["y"]
		}
		if label := strings.TrimSpace(p["on_timeout_goto"]); label != "" {
			idx, err := r.jumpLabel(label)
			return idx, true, err
		}
	case "capture_bitmap":
		if name := strings.TrimSpace(p["name"]); name != "" {
			r.vars[name] = "[bitmap-capture-placeholder]"
		}
	case "capture_text", "capture_text_ocr":
		if name := strings.TrimSpace(p["name"]); name != "" {
			r.vars[name] = r.resolveValue(p["fallback_text"])
		}
	case "capture_barcode", "capture_qr":
		if name := strings.TrimSpace(p["name"]); name != "" {
			r.vars[name] = r.resolveValue(p["fallback_code"])
		}
	case "scrape_webpage":
		url := r.resolveValue(p["url"])
		if url == "" {
			return 0, false, fmt.Errorf("scrape_webpage sem url")
		}
		...
		out := body
		if rx := strings.TrimSpace(p["regex"]); rx != "" {
	...
	case "smart_click", "ai_object_search", "phrase_insertion", "window_focus", "embed_macro", "post_playback":
		if t == "embed_macro" {
			path := r.resolveValue(p["path"])
			if path != "" {
				child, err := loadScript(path)
				if err != nil {
					return 0, false, err
				}
				childRunner := &Runner{script: child, vars: r.vars, labels: buildLabels(child), repeatCount: map[int]int{}}
				if err := childRunner.run(); err != nil {
					return 0, false, err
				}
			}
		}
	case "label", "comment", "group_start", "group_end", "breakpoint", "disable_action":
		// metadados/no-op no executor console
	default:
